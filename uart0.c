...
{
    Module_Clk_init(PORTA);        /* Enable clock for GPIO PORTA */
    while(!(PRGPIO & 0x01));   /* Wait until GPIO PORTA clock is activated and it is ready for access*/
	
		GPIOAMSEL(PORTA) 	&= 0xFC;		 /* Disable Analog on PA0 & PA1 */
		GPIODIR(PORTA) 		&= 0xFE;		/* Configure PA0 as input pin */
		GPIODIR(PORTA) 		|= 0x02; 		/* Configure PA1 as output pin */
		GPIOAFSEL(PORTA) 	|= 0x03; 	/* Enable alternative function on PA0 & PA1 */
		/* Set PMCx bits for PA0 & PA1 with value 1 to use PA0 as UART0 RX pin and PA1 as UART0 Tx pin */	
		GPIOPCTL(PORTA) 	= (GPIOPCTL(PORTA) & 0xFFFFFF00) | 0x00000011;			 
		GPIODEN(PORTA) 			|= 0x03;         /* Enable Digital I/O on PA0 & PA1 */	 
}
/*******************************************************************************
...
void UART0_Init(void)
{
   ...
   Module_Clk_init(PORTA);          /* Enable clock for UART0 */
